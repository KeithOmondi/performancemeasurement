...
const ApprovalIdModalPage = ({ indicatorId, onClose }: Props) => {
  ...
  const canReview =
    !isCompleted && indicator?.status === "Awaiting Super Admin";
  ...
  const previewPct =
    target > 0 ? Math.min(100, (previewProgress / target) * 100) : 0;
  const currentPct =
    target > 0 ? Math.min(100, (currentProgress / target) * 100) : 0;
  ...
  const handleReview = async () => {
    ...
    if (decision === "approve") {
      ...
      if (!progressValue.trim() || isNaN(parsed) || parsed <= 0) {
        setProgressError("Enter the value being approved.");
        hasError = true;
      } else if (!isAnnual && parsed > remainingNeeded) {
        setProgressError(`Max approvable is ${remainingNeeded} ${unit}.`);
        hasError = true;
      } else if (isAnnual && parsed !== target) {
        setProgressError(
          `Annual certification requires exactly ${target} ${unit}.`
        );
        hasError = true;
      }
      if (needsNextDeadline && !nextDeadline) {
        setDeadlineError(
          "Set the next submission deadline for the remaining work."
        );
        hasError = true;
      }
    }
          ...
          <div className="px-6 pt-5 pb-4 border-b border-slate-100 shrink-0 bg-white">
            <div className="flex items-start justify-between gap-4">
              <div className="flex items-start gap-3 min-w-0">
                <div
                  className={`mt-0.5 p-2 rounded-xl shrink-0 ${
                    isAnnual ? "bg-amber-100" : "bg-blue-100"
                  }`}
                >
                  {isAnnual ? (
              ...
              <div className="flex items-center gap-2 shrink-0">
                <span
                  className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full border text-[9px] font-black uppercase tracking-wider ${statusColor}`}
                >
                  <StatusIcon size={11} />
                  {indicator.status}
                </span>
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-4 border-r border-slate-100">
              ...
              <div className="bg-slate-50 rounded-xl p-4">
                <div className="flex items-end justify-between mb-2">
                  ...
                  {decision === "approve" && parsedProgress > 0 && (
                    <div className="text-right">
                      <p className="text-[8px] font-black text-emerald-500 uppercase tracking-wider">
                        After Approval
                      </p>
                      <p className="text-sm font-bold text-emerald-600">
                        {previewProgress} {unit}
                        {previewProgress >= target && (
                          <span className="ml-1 text-[9px]">✓ Complete</span>
                        )}
                      </p>
                    </div>
                  )}
                ...
                <div className="flex justify-between mt-1">
                  <span className="text-[8px] text-slate-400">
                    {currentPct.toFixed(0)}% complete
                  </span>
                  {!isFullyComplete && (
                    <span className="text-[8px] text-amber-500">
                      {remainingNeeded} {unit} remaining
                    </span>
                  )}
                </div>
              </div>
              ...
              {allSubmissions.length > 0 && (
                <div className="space-y-3">
                  ...
                  {allSubmissions.map((sub, idx) => {
                    ...
                    const quarterLabel = sub.quarter
                      ? `Q${sub.quarter}`
                      : "Annual";
                    ...
                    const chip = statusChip[sub.reviewStatus] ?? {
                      label: sub.reviewStatus,
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          ...
                          <span
                            className={`ml-auto text-[8px] font-black px-2 py-0.5 rounded-full uppercase tracking-wide ${chip.cls}`}
                          >
                            {chip.label}
                          </span>
                        </div>
                            ...
                            <div className="flex flex-col gap-2">
                              {(sub.documents as IDocument[]).map((doc, di) => (
                                <button
                                  ...
                                  className="flex items-start gap-3 p-2.5 rounded-lg border border-slate-100 hover:border-[#2c5f2e]/30 hover:bg-[#f0f7f0] transition-all group text-left bg-white w-full"
                                >
                                  {/* File icon */}
                                  <div className="w-8 h-8 rounded-md bg-slate-100 group-hover:bg-white flex items-center justify-center shrink-0 transition-colors mt-0.5">
                                  ...
                                  <ExternalLink
                                    size={10}
                                    className="text-slate-300 group-hover:text-[#2c5f2e] shrink-0 transition-colors mt-1"
                                  />
                                </button>
                              ))}
                            </div>
              ...
              <div className="border border-slate-100 rounded-xl p-4 flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-[11px] font-bold text-[#1a3a32] truncate">
                ...
                <div>
                  <p
                    className={`text-[9px] font-black uppercase tracking-wider ${
                      isAnnual ? "text-amber-700" : "text-blue-700"
                    }`}
                  >
              ...
              {indicator.status === "Rejected by Super Admin" && (
                <div className="rounded-xl bg-rose-50 border border-rose-100 p-4 text-center">
                  ...
                  <p className="text-[9px] text-slate-500 mt-1">
                    Awaiting resubmission from assignee.
                  </p>
                </div>
              )}
                ...
                <>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <button
                        onClick={() =>
                          setDecision(decision === "approve" ? null : "approve")
                        }
                      ...
                      <button
                        onClick={() =>
                          setDecision(decision === "reject" ? null : "reject")
                        }
                        className={`flex items-center justify-center gap-1.5 py-2.5 rounded-xl border text-[10px] font-black uppercase tracking-wider transition-all ${
                          decision === "reject"
                    ...
                    <div className="space-y-3">
                      <div>
                        ...
                        <div className="mb-2">
                          <input
                            ...
                            step={
                              isAnnual
                                ? target
                                : Math.max(1, Math.floor(remainingNeeded / 10))
                            }
                          ...
                          <div className="flex justify-between text-[8px] text-slate-400 mt-0.5">
                            ...
                            {!isAnnual && !isFullyComplete && (
                              <button
                                type="button"
                                onClick={() => {
                                  setProgressValue(String(remainingNeeded));
                                  if (progressError) setProgressError("");
                                }}
                            ...
                            <span>
                              {isAnnual ? target : remainingNeeded} {unit}
                            </span>
                          </div>
                        </div>
                        {/* Number input */}
                        ...
                        {parsedProgress > 0 && !progressError && (
                          <div
                            className={`mt-2 p-2 rounded-lg text-[9px] font-medium ${
                              previewProgress >= target
                          ...
                          >
                            {previewProgress >= target ? (
                              <span className="flex items-center gap-1">
                                <CheckCircle2 size={10} /> This will fully
                                complete the indicator.
                              </span>
                            ) : (
                              <span className="flex items-center gap-1">
                                <ChevronRight size={10} />{" "}
                                {Math.max(0, remainingNeeded - parsedProgress)}{" "}
                                {unit} will remain after this approval.
                              </span>
                            )}
                          </div>
                        )}
                      ...
                      {needsNextDeadline && (
                        <div>
                          ...
                          <p className="text-[8px] text-slate-400 mt-1">
                            Assignee must resubmit the remaining{" "}
                            {Math.max(0, remainingNeeded - parsedProgress)}{" "}
                            {unit} by this date.
                          </p>
                        </div>
                  ...
                  {decision === "reject" && (
                    <div>
                      ...
                      <p className="text-[8px] text-slate-400 mt-1.5">
                        ...
                        {indicator.assigneeDisplayName || "the assignee"}.
                      </p>
                    </div>
                  )}
