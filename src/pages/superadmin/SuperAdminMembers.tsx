...
const SuperAdminMembers = () => {
  ...
  const { indicators, loading: indicatorsLoading }           = useSelector((s: RootState) => s.indicators);
  const { plans }                                            = useSelector((s: RootState) => s.strategicPlan);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    let result;
    if (isEditing) {
      // ✅ Use _id (normalized) for edit/delete operations
  ...
  // ✅ Use flat assigneeId field to match indicators to users
  const getMemberIndicators = (userId: string) =>
    indicators.filter((ind) => ind.assignee === userId);

  const getActivityDetails = (activityId: string) => {
    for (const plan of plans) {
      for (const objective of plan.objectives) {
        const activity = objective.activities.find((a) => a.id === activityId);
      ...
      {isLoading && users.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filteredUsers.map((member) => {
            // ✅ _id is normalized from PG id by userService
            ...
            <form onSubmit={handleSubmit} className="p-10 space-y-6">
              ...
              <div className="space-y-1">
                ...
                <div className="relative">
                  ...
                  <input required className="w-full pl-12 pr-4 py-3.5 bg-slate-50 border border-slate-200 rounded-2xl text-sm outline-none focus:border-[#1d3331] transition-all"
                    placeholder="Full Official Name" value={formData.name}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <input required className="w-full px-4 py-3.5 bg-slate-50 border border-slate-200 rounded-2xl text-sm focus:border-[#1d3331] outline-none"
                    placeholder="PF-XXXX" value={formData.pjNumber}
                ...
                <div className="space-y-1">
                  ...
                  <input required className="w-full px-4 py-3.5 bg-slate-50 border border-slate-200 rounded-2xl text-sm focus:border-[#1d3331] outline-none"
                    placeholder="Position" value={formData.title}
              ...
              <div className="space-y-1">
                ...
                <div className="relative">
                  ...
                  <input required type="email" className="w-full pl-12 pr-4 py-3.5 bg-slate-50 border border-slate-200 rounded-2xl text-sm focus:border-[#1d3331] outline-none"
                    placeholder="email@judiciary.go.ke" value={formData.email}
              ...
              <div className="space-y-1">
                ...
                <div className="relative">
                  ...
                  <select className="w-full pl-12 pr-4 py-3.5 bg-slate-50 border border-slate-200 rounded-2xl text-sm outline-none appearance-none cursor-pointer focus:border-[#1d3331]"
                ...
                <div className="space-y-4">
                  ...
                  {getMemberIndicators(selectedMember._id).length > 0 ? (
                    getMemberIndicators(selectedMember._id).map((ind) => {
                      ...
                      return (
                        <div key={ind.id} className="p-5 bg-slate-50 rounded-[1.5rem] border border-slate-100 hover:border-emerald-200 transition-colors">
