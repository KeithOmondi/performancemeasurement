...
import SuperAdminApprovedModal from "./SuperAdminApprovedModal"; // ✅ use dedicated modal
...
const SuperAdminApproved = () => {
  ...
  const approvedItems = useMemo(() => {
    return superAdminApprovedIndicators.filter((ind) => {
      const matchesSearch =
  ...
  }, [superAdminApprovedIndicators, searchTerm]);
    ...
    <div className="p-6 md:p-10 bg-[#fcfdfb] min-h-screen font-sans">
            ...
            <div>
              ...
              <div className="flex gap-2 mt-2">
                <span className="bg-emerald-50 text-emerald-700 text-[9px] px-3 py-1 rounded-lg font-black border border-emerald-100 uppercase tracking-widest">
                  {approvedItems.length} Certified Performance Records
                </span>
              </div>
            </div>
          </div>
      ...
      {approvedItems.length === 0 ? (
        <div className="bg-white rounded-[0.5rem] py-40 text-center border border-dashed border-slate-200 shadow-2xl shadow-slate-200/30">
          ...
          <h2 className="text-sm font-black text-slate-400 uppercase tracking-widest">No certified records found</h2>
        </div>
      ) : (
              ...
              <tbody className="divide-y divide-slate-50">
                {approvedItems.map((indicator) => {
                  ...
                  const superEntry = [...history].reverse().find(
                    (h) => h.reviewerRole === "superadmin" && h.action === "Approved"
                  );
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-10 py-7 text-right">{/* Empty intentionally */}</td>
                    </tr>
                  );
                })}
              </tbody>
      ...
      {/* ✅ Use the dedicated Super Admin Modal */}
      {selectedIndicatorId && (
