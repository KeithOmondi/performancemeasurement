...
const SuperAdminAssign = ({ onClose }: { onClose: () => void }) => {
  ...
  // Individual mode: single user id
  const [selectedUserId, setSelectedUserId] = useState<string>("");
  // Team mode: single team id
  ...
  const isAssigneeSelected = assignmentType === "Individual" ? !!selectedUserId : !!selectedTeamId;
  ...
  const handleAssign = async () => {
    ...
    const payload: Partial<IIndicator> = {
      ...
      assignee: assignmentType === "Individual" ? selectedUserId : selectedTeamId as any,
                    ...
                    <div className="mt-3 p-4 bg-[#1a3a32]/5 rounded-2xl border border-[#1a3a32]/10 space-y-3">
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        {selectedTeam.members.length > 6 && (
                          <span className="text-[10px] text-slate-400 font-bold ml-1">+{selectedTeam.members.length - 6} more</span>
                        )}
                      </div>
                    </div>
                  )}
