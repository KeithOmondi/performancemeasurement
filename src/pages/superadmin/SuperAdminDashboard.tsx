...
const SuperAdminDashboardPage = () => {
  ...
  const { users = [], isLoading: uLoad } = useSelector(
    (s: RootState) => s.users,
  );
  const {
    indicators = [],
  ...
  const analytics = useMemo(() => {
    ...
    const pending = indicators.filter(
      (i) =>
        i.status?.toLowerCase().includes("awaiting") ||
        i.status?.toLowerCase().includes("pending"),
    ).length;
    const approved = indicators.filter(
      (i) =>
        i.status === "Completed" ||
        ...
        if (plan.perspective?.trim().toLowerCase() === name.toLowerCase()) {
          plan.objectives?.forEach((obj) => {
            ...
            obj.activities?.forEach((act) => {
              ...
              if (indicators.some((ind) => ind.activityId === act.id))
                assignedActs++;
            });
          });
        }
    ...
    indicators.forEach((ind) => {
      if (ind.assignee) {
        userAssignments[ind.assignee] =
          (userAssignments[ind.assignee] || 0) + 1;
      }
    });
  ...
  const statCards = [
    {
      label: "TOTAL SUB-INDICATORS",
      val: analytics.kpis.totalPossible,
      color: "bg-[#1d3331]",
      sub: "",
      filter: "ALL",
    },
    ...
    <div className="min-h-screen bg-[#fcfcf7] p-4 md:p-8 text-[#1a2c2c] font-sans">
      ...
      <div className="bg-[#fff9e6] border border-[#f5e6b3] rounded-lg p-3 mb-8 flex items-center gap-3">
        ...
        <p className="text-[12px] font-medium text-amber-900">
          <span className="font-bold">{analytics.kpis.pending} indicators</span>{" "}
          have evidence pending review ·{" "}
          <span className="font-bold ml-1">
            {analytics.kpis.overdue} indicators
          </span>{" "}
      ...
      <div className="space-y-6 mb-12">
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
          {statCards.map((item, i) => (
            <Link
              ...
              className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm flex flex-col justify-between min-h-[140px] hover:border-[#1d3331] transition-all cursor-pointer"
            >
              <div>
                <h2
                  className={`text-4xl font-serif font-bold ${item.valColor || "text-[#1d3331]"}`}
                >
                  {item.val}
                </h2>
                <p className="text-[9px] font-black uppercase text-slate-400 tracking-wider mt-1">
                  {item.label}{" "}
                  <span className="text-[8px] opacity-60 lowercase">
                    {item.sub}
                  </span>
                </p>
              </div>
              <div className="w-full bg-slate-100 h-[3px] rounded-full overflow-hidden mt-4">
                <div
                  ...
                  style={{
                    width: `${(item.val / (analytics.kpis.totalPossible || 1)) * 100}%`,
                  }}
                />
              </div>
            </Link>
        ...
        <div className="grid grid-cols-1 md:grid-cols-5">
          <Link
            to="/superadmin/indicators?filter=ALL"
            className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm flex flex-col justify-between min-h-[140px] hover:border-red-800 transition-all cursor-pointer"
          >
            <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-white p-6 rounded-2xl border-l-[5px] border-[#1d3331] border-slate-100 shadow-sm flex flex-col justify-center min-h-[100px]">
            <h2 className="text-4xl font-serif font-bold text-[#1d3331]">
              {analytics.kpis.approved}
            </h2>
            <p className="text-[9px] font-black uppercase text-slate-400 tracking-wider mt-1">
              REVIEWED & APPROVED
            </p>
        ...
        <div className="lg:col-span-8">
          ...
          <div className="space-y-3">
            {queue.slice(0, 5).map((item, idx) => (
              <div
                key={idx}
                className="bg-white p-5 rounded-2xl border border-slate-100 flex items-center justify-between group hover:border-[#1d3331] transition-all"
              >
                <div className="flex gap-4">
                  <div
                    className={`w-2 h-2 rounded-full mt-2 ${item.status === "Overdue" ? "bg-red-600" : "bg-amber-500"}`}
                  />
                  <div>
                    ...
                    <p className="text-[10px] text-slate-400 mt-1 uppercase font-bold tracking-tighter italic">
                      {item.submittedOn} · {item.documentsCount || 0} documents
                      uploaded
                    </p>
                  </div>
                </div>
          ...
          {analytics.perspectives.map((p, idx) => (
            ...
            >
              <div className="flex justify-between items-center mb-1">
                ...
                <span className="text-[10px] font-black bg-emerald-50 text-emerald-800 px-2 py-0.5 rounded-full">
                  {p.val}%
                </span>
              </div>
              ...
              <div className="w-full bg-slate-50 h-2 rounded-full overflow-hidden">
                <div
                  className="bg-[#1d3331] h-full group-hover:bg-emerald-700 transition-all"
                  style={{ width: `${p.val}%` }}
                />
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {users.slice(0, 4).map((member) => {
            ...
            return (
              <div
                key={key}
                className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm group hover:shadow-md transition-all relative"
              >
                <div className="flex items-center gap-4 mb-6">
                  ...
                  <div>
                    <h4 className="text-[15px] font-serif font-bold leading-tight text-[#1a2c2c] group-hover:text-emerald-700">
                      {member.name}
                    </h4>
                ...
                <div className="space-y-3 mb-6">
                  ...
                  <div className="flex items-center gap-2 text-[11px] font-bold text-emerald-700">
                    ...
                    <FileText size={14} /> {analytics.userAssignments[key] || 0}{" "}
                    assigned
                  </div>
                </div>
                <div className="flex gap-2">
                  <button className="flex-1 bg-slate-50 text-[10px] font-black uppercase py-2.5 rounded-xl hover:bg-slate-100">
                    Profile
                  </button>
