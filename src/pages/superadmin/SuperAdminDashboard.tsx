...
const SuperAdminDashboardPage = () => {
  ...
  const analytics = useMemo(() => {
    const totalPossible = plans.reduce((acc, p) => acc + (p.objectives?.reduce((oAcc, obj) => oAcc + (obj.activities?.length || 0), 0) || 0), 0);
    const assigned = indicators.length;
    const pending = indicators.filter(ind => ind.status?.toLowerCase().includes("awaiting") || ind.status?.toLowerCase().includes("pending")).length;
    const approved = indicators.filter(ind => ind.status === "Completed" || ind.status?.toLowerCase().includes("approved")).length;
    const rejected = stats?.general?.rejected || 0;
    const overdue = stats?.general?.overdue || 0;

    // Perspective & User Logic
    const categories = ["Core Business / Mandate", "Customer Perspective", "Finance Perspective", "Innovation & Learning", "Internal Process"];
        ...
        if (plan.perspective?.trim().toLowerCase() === name.toLowerCase()) {
          plan.objectives?.forEach((obj) => {
            ...
            obj.activities?.forEach((act) => {
              if (indicators.some((ind) => ind.activityId === act._id)) assignedActs++;
            });
          });
        }
      });
      return { name, total: totalActs, assigned: assignedActs, val: totalActs > 0 ? Math.round((assignedActs / totalActs) * 100) : 0 };
    });

    const userAssignments: Record<string, number> = {};
    indicators.forEach((ind) => {
      const userId = typeof ind.assignee === "string" ? ind.assignee : ind.assignee?._id;
      if (userId) userAssignments[userId] = (userAssignments[userId] || 0) + 1;
    });

    return { perspectives, userAssignments, kpis: { totalPossible, assigned, pending, approved, overdue, rejected } };
  ...
  return (
    <div className="min-h-screen bg-[#fcfcf7] p-4 md:p-8 text-[#1a2c2c] font-sans">
      ...
      {/* 🔹 STATS SECTION (Exact Match to Stat.png) */}
      <div className="space-y-6 mb-12">
        ...
        {/* Row 3: Wide Approved/Rejected Bars */}
      ...
      {/* 🔹 LOWER CONTENT: ACTIVITY & PERSPECTIVES */}
          ...
          <div className="space-y-3">
            {queue.slice(0, 5).map((item, idx) => (
              <div key={idx} className="bg-white p-5 rounded-2xl border border-slate-100 flex items-center justify-between group hover:border-[#1d3331] transition-all">
                ...
                <Link to={`/superadmin/submissions?id=${item._id}`} className="text-emerald-700 text-[11px] font-bold uppercase border-b border-transparent hover:border-emerald-700 pb-0.5 transition-all">Review</Link>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="mt-16 pt-8 border-t border-slate-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {users.slice(0, 4).map((member) => (
            <div key={member._id} className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm group hover:shadow-md transition-all relative">
              ...
              <div className="space-y-3 mb-6">
                <div className="flex items-center text-[10px] font-bold text-slate-500 uppercase">PF: {member.pjNumber || '---'}</div>
                <div className="flex items-center gap-2 text-[11px] font-bold text-emerald-700">
                  <FileText size={14} /> {analytics.userAssignments[member._id] || 0} assigned
                </div>
              </div>
