...
// Ensure local interfaces match your slice types to avoid 'any'
interface IUser {
  id?: string;
  _id?: string;
  name: string;
...
const SuperAdminDashboardPage = () => {
  ...
  const analytics: AnalyticsResult = useMemo(() => {
    // 1. Calculate Grand Totals from Plans
    const totalPossible = plans.reduce((acc, p) => 
      acc + (p.objectives?.reduce((oAcc, obj) => oAcc + (obj.activities?.length || 0), 0) || 0), 0
    );

    // 2. Status Breakdown
    const assigned = indicators.length;
    const pending = indicators.filter(i => 
      i.status?.toLowerCase().includes("awaiting") || i.status?.toLowerCase().includes("pending")
    ).length;
    
    const approved = indicators.filter(i => 
      i.status === "Completed" || i.status?.toLowerCase().includes("approved")
    ).length;
        ...
        if (plan.perspective?.trim().toLowerCase() === name.toLowerCase()) {
          plan.objectives?.forEach((obj) => {
            ...
            obj.activities?.forEach((act) => {
              // Safe ID check without 'any'
              const activityId = act.id || (act as { _id?: string })._id;
              if (indicators.some((ind) => ind.activityId === activityId)) {
                assignedActs++;
              }
            });
          });
  ...
  const statCards = [
    { label: "TOTAL SUB-INDICATORS", val: analytics.kpis.totalPossible, color: "bg-[#1d3331]", sub: "", filter: "ALL" },
    { label: "ASSIGNED", val: analytics.kpis.assigned, color: "bg-[#1d3331]", sub: `(OF ${analytics.kpis.totalPossible})`, filter: "ASSIGNED" },
    { 
        label: "UNASSIGNED", 
        val: analytics.kpis.totalPossible - analytics.kpis.assigned, 
        color: "bg-[#1d3331]", 
        sub: `(OF ${analytics.kpis.totalPossible})`, 
        valColor: "text-red-800", 
        filter: "UNASSIGNED" 
    },
    { label: "AWAITING REVIEW", val: analytics.kpis.pending, color: "bg-amber-500", sub: "", valColor: "text-amber-600", filter: "ALL" },
    { label: "APPROVED", val: analytics.kpis.approved, color: "bg-[#1d3331]", sub: "", filter: "ALL" },
  ];
  ...
  if (uLoad || sLoad || iLoad || pLoad || !stats) {
    return (
      <div className="min-h-screen bg-[#fcfcf7] flex items-center justify-center flex-col">
        ...
        <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 italic">Syncing Registry intelligence...</p>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-[#fcfcf7] p-4 md:p-8 text-[#1a2c2c] font-sans">
      ...
      <div className="space-y-6 mb-12">
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
          {statCards.map((item, i) => (
            <Link
              key={i}
              to={`/superadmin/indicators?filter=${item.filter}`}
              className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm flex flex-col justify-between min-h-[140px] hover:border-[#1d3331] transition-all cursor-pointer group"
            >
              <div>
              ...
              <div className="w-full bg-slate-100 h-[3px] rounded-full overflow-hidden mt-4">
                <div 
                  ...
                  style={{ width: `${analytics.kpis.totalPossible > 0 ? (item.val / analytics.kpis.totalPossible) * 100 : 0}%` }} 
                />
              </div>
            </Link>
          ))}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Link to="/superadmin/indicators?filter=ALL" className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm hover:border-red-800 transition-all">
             <h2 className="text-4xl font-serif font-bold text-red-800">{analytics.kpis.overdue}</h2>
             <p className="text-[9px] font-black uppercase text-slate-400 tracking-wider mt-1">OVERDUE DEADLINES</p>
          </Link>
          <div className="bg-white p-6 rounded-2xl border-l-[5px] border-[#1d3331] shadow-sm">
          ...
          <div className="bg-white p-6 rounded-2xl border-l-[5px] border-red-700 shadow-sm">
             ...
             <p className="text-[9px] font-black uppercase text-slate-400 tracking-wider mt-1">RETURNED FOR CORRECTION</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-12">
        <div className="lg:col-span-8">
          <div className="flex justify-between items-center mb-6">
            ...
            <Link to="/superadmin/submissions" className="text-[10px] font-black bg-[#1d3331] text-white px-4 py-2 rounded uppercase tracking-widest flex items-center gap-2">
          ...
          <div className="space-y-3">
            {queue.slice(0, 5).map((item, idx) => (
              <div key={idx} className="bg-white p-5 rounded-2xl border border-slate-100 flex items-center justify-between group hover:border-[#1d3331] transition-all">
                ...
                <Link to={`/superadmin/submissions?id=${item.id}`} className="text-emerald-700 text-[11px] font-bold uppercase hover:border-b hover:border-emerald-700 transition-all">Review</Link>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="lg:col-span-4 space-y-4">
          ...
          {analytics.perspectives.map((p, idx) => (
            <div key={idx} className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm group">
      ...
      <div className="mt-16 pt-8 border-t border-slate-200">
        <div className="flex justify-between items-center mb-8">
          ...
          <Link to="/superadmin/team" className="text-[10px] font-black border border-slate-300 px-5 py-2 rounded-xl uppercase hover:bg-[#1d3331] hover:text-white transition-all">View All →</Link>
        </div>
            ...
            return (
              <div key={userId} className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm group hover:shadow-md transition-all">
                ...
                <div className="space-y-3 mb-6">
                  <div className="text-[10px] font-bold text-slate-500 uppercase">PF: {member.pjNumber || "---"}</div>
                  <div className="flex items-center gap-2 text-[11px] font-bold text-emerald-700">
                    <FileText size={14} /> {analytics.userAssignments[userId] || 0} assigned
                  </div>
                </div>
                <div className="flex gap-2">
                  ...
                  <button className="p-2.5 bg-slate-50 rounded-xl hover:bg-slate-100"><Mail size={14} className="text-slate-400" /></button>
                </div>
              </div>
            );
          })}
