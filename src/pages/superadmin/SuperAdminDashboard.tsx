...
import { ChevronRight, Loader2, Users, Target, AlertCircle, CheckCircle, Clock } from "lucide-react";
...
const SuperAdminDashboardPage = () => {
  ...
  /**
   * Refined status config to match the "Recent Activity" visual cues
   */
  const getStatusConfig = (status: string) => {
    switch (status) {
      case "Awaiting Super Admin":
      case "Awaiting Admin Approval":
        return { dot: "bg-amber-500", action: "Review", actionColor: "text-[#c2a336]" };
      case "Rejected by Admin":
      case "Rejected by Super Admin":
      case "Overdue":
        return { dot: "bg-red-600", action: "View", actionColor: "text-red-700" };
      case "Completed":
      case "Approved":
        return { dot: "bg-emerald-600", action: "View", actionColor: "text-emerald-700" };
      default:
        return { dot: "bg-slate-400", action: "Manage", actionColor: "text-slate-600" };
    }
  };
    ...
    <div className="min-h-screen bg-[#fcfcf7] p-4 md:p-8 text-[#1a2c2c] font-sans">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 mb-4">
        {[
          ...
          { label: "ASSIGNED", value: assignedCount, border: "border-slate-300", path: "/superadmin/indicators?filter=ASSIGNED" },
          ...
          { label: "AWAITING REVIEW", value: awaitingReviewCount, border: "border-yellow-500", path: "/superadmin/submissions" },
          { label: "APPROVED", value: certifiedCount || stats?.general?.approved || 0, border: "border-[#1d3331]", path: "/superadmin/indicators?filter=REVIEWED" },
        ].map((stat, i) => (
          <Link 
            key={i} 
            to={stat.path}
      ...
      <div className="flex flex-col gap-4 mb-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <Link 
            ...
            className="lg:col-span-2 bg-white p-5 rounded-xl border-l-[6px] border-[#1d3331] shadow-sm flex items-center h-32 hover:bg-slate-50 transition-colors"
          >
            <CheckCircle className="text-[#1d3331] mr-6 opacity-20" size={48} />
            <div>
              <span className="text-4xl md:text-5xl font-serif font-bold text-[#1d3331] leading-none">{certifiedCount}</span>
              <p className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] mt-1">Total Approved Registry Records</p>
            </div>
          </Link>
          ...
          >
            ...
            <div>
              ...
              <p className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] mt-1">Total Rejected / Returned Dossiers</p>
            </div>
          </Link>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        {/* 🔹 REGISTRY QUEUE - Updated to match screenshot styling */}
        <div className="lg:col-span-8">
          <div className="flex items-center justify-between mb-6 border-b border-slate-100 pb-4">
            <div>
              <h3 className="text-xl font-serif font-bold text-[#1d3331]">Recent Submissions</h3>
              <p className="text-[10px] text-slate-400 uppercase tracking-widest font-bold">Certification Pipeline</p>
            </div>
            <Link to="/superadmin/submissions" className="text-[9px] font-black bg-[#1d3331] text-white px-4 py-2 rounded-lg uppercase tracking-tighter hover:bg-emerald-900 transition-colors shadow-sm">
              Open Full Queue
            </Link>
          </div>
          ...
          <div className="space-y-3">
            {queue.slice(0, 5).map((item, idx) => {
              ...
              // Determine if item is Annual or Quarterly based on indicator cycle
              const isAnnual = item.indicatorTitle?.toLowerCase().includes("annual");
              const cycleLabel = isAnnual ? "Annual" : `Q${item.quarter}`;

              return (
                <div key={item._id || idx} className="bg-white p-5 rounded-2xl border border-slate-200 flex flex-col sm:flex-row sm:items-center justify-between shadow-sm gap-4 group hover:shadow-md transition-all">
                  <div className="flex items-start space-x-4">
                    <div className={`w-2.5 h-2.5 rounded-full ${config.dot} mt-2 flex-shrink-0`} />
                    <div className="text-[14px]">
                      <p className="text-slate-600 font-medium leading-relaxed">
                        <span className="font-bold text-[#1a2c2c]">{item.submittedBy || "System Lead"}</span> 
                        {item.status === 'Overdue' ? ' deadline passed for ' : ' submitted evidence for '}
                        <span className="font-bold text-[#1a2c2c]">{item.indicatorTitle || item.activityDescription}</span> 
                        <span className="text-slate-400 mx-2">→</span>
                        <span className={`font-bold ${config.actionColor}`}>{config.action}</span>
                      </p>
                      
                      <div className="flex items-center gap-3 mt-1.5">
                        <p className="text-[11px] text-slate-400 font-medium">
                          {item.submittedOn || "Recent"} · {item.documentsCount || 0} document{item.documentsCount !== 1 ? 's' : ''} uploaded
                        </p>
                        
                        {/* Cycle context badge */}
                        <span className="text-[9px] bg-slate-100 text-slate-500 px-2 py-0.5 rounded font-black uppercase tracking-tighter border border-slate-200">
                          {cycleLabel}
                        </span>
                      </div>
                    </div>
                  ...
                  <Link 
                    ...
                    className="text-[#1d3331] font-black hover:underline uppercase text-[10px] tracking-widest self-end sm:self-center bg-slate-50 px-4 py-2 rounded-lg border border-slate-100 hover:bg-white transition-colors"
                  >
                    {config.action}
                  </Link>
                </div>
            ...
            {queue.length === 0 && (
              <div className="text-center py-12 bg-white rounded-2xl border-2 border-dashed border-slate-100">
                ...
                <p className="text-[10px] font-black uppercase tracking-widest text-slate-300">Queue Cleared: No Pending Actions</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="lg:col-span-4">
          <div className="mb-6 border-b border-slate-100 pb-4">
            ...
            <p className="text-[10px] text-slate-400 uppercase tracking-widest font-bold">Perspective Performance</p>
          </div>
          <div className="space-y-4">
            {stats?.perspectiveStats?.map((p, idx) => (
              ...
              >
                ...
                <p className="text-[9px] font-bold text-slate-400 mb-3 uppercase tracking-tighter">{p.count} Active Indicators</p>
      ...
      <div className="mt-12 pt-8 border-t border-slate-200">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
          ...
          <Link to="/superadmin/team" className="text-[10px] font-black border-2 border-[#1d3331] text-[#1d3331] px-5 py-2 rounded-xl uppercase tracking-[0.1em] flex items-center hover:bg-[#1d3331] hover:text-white transition-all shadow-sm">
            MANAGE USERS <ChevronRight size={14} className="ml-2" />
          </Link>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-6">
          {displayedUsers.map((member) => (
            <div key={member._id} className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-all">
              <div className="flex flex-col items-center text-center">
                <div className="w-14 h-14 rounded-full bg-[#1d3331] text-white flex items-center justify-center font-black mb-4 text-sm shadow-xl shadow-[#1d3331]/10 border-4 border-white">
                ...
                <p className="text-[9px] font-black text-emerald-700 uppercase tracking-widest mb-4">
                  {member.role === 'superadmin' ? 'SYSTEM REGISTRAR' : (member.title || "STAFF")}
                </p>
          ...
          <Link to="/superadmin/team" className="bg-[#f0f0e8]/30 border-2 border-dashed border-slate-200 rounded-2xl flex items-center justify-center cursor-pointer hover:bg-[#f0f0e8]/60 transition-all min-h-[200px]">
            <div className="text-center px-4">
              <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">
                {remainingCount > 0 ? `+${remainingCount} Personnel Hidden` : "Full Directory"}
              </p>
              <span className="text-[9px] font-bold text-emerald-800 uppercase underline mt-2 block">View All Records</span>
            </div>
          </Link>
        </div>
      </div>
