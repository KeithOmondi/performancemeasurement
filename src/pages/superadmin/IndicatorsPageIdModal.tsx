...
import {
  ...
  RotateCcw, Loader2, Lock, User, Paperclip
} from "lucide-react";
    ...
    <div className="bg-[#fcfcf7] w-full h-full flex flex-col shadow-2xl overflow-hidden font-sans relative">
      ...
      <main className="flex-1 overflow-y-auto bg-white">
        <div className="max-w-3xl mx-auto py-8 px-8 space-y-12">
          ...
          {/* INDICATOR DETAILS SECTION - Matches Styling in Screenshot 2 */}
          ...
          {/* ASSIGNMENT SECTION - Matches Screenshot 2 */}
          <section className="space-y-6">
            <h3 className="text-[11px] font-black text-slate-500 uppercase tracking-[0.15em] border-b border-slate-100 pb-3">
              Assignment
            </h3>
          ...
          {/* EVIDENCE SECTION - Matches Drag & Drop styling in Screenshot 2 */}
              ...
              <div className="space-y-6 animate-in fade-in duration-700">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-3">
                    ...
                    <input type="number" value={progressOverride} onChange={(e) => setProgressOverride(Number(e.target.value))} className="w-full p-4 bg-black/40 border border-white/10 rounded-2xl font-black text-lg text-white outline-none focus:border-[#c2a336] transition-all" />
                  </div>
                  {!isLastQuarter && (
                    <div className="space-y-3">
                      <label className="text-[10px] font-black uppercase tracking-[0.2em] text-amber-400">Next Q Deadline</label>
                      <input type="date" min={todayStr} value={nextDeadline} onChange={(e) => setNextDeadline(e.target.value)} className="w-full p-4 bg-black/40 border border-amber-500/30 rounded-2xl font-bold text-white outline-none focus:border-amber-400 transition-all" />
                    </div>
                  )}
                </div>
                ...
                {showRejectReason && (
                  <div className="space-y-3 animate-in slide-in-from-top-4 duration-500">
                    ...
                    <textarea value={decisionReason} onChange={(e) => setDecisionReason(e.target.value)} className="w-full p-5 bg-black/40 border border-rose-500/30 rounded-2xl text-sm font-medium outline-none min-h-[100px]" placeholder="Explain the corrections required..." />
                  </div>
                )}

                <div className="flex flex-col md:flex-row gap-4 pt-2">
                  <button onClick={() => handleCertification("Approved")} disabled={isProcessing || !activeSubmission} className="flex-[2] py-5 bg-[#c2a336] text-[#1d3331] rounded-2xl font-black text-[11px] uppercase tracking-[0.25em] hover:bg-white hover:scale-[1.02] transition-all flex items-center justify-center gap-3 disabled:opacity-30">
                    <CheckCircle2 size={18} /> {isLastQuarter ? "Certify Performance" : "Approve & Open Next Period"}
                  </button>
                  <button onClick={() => handleCertification("Rejected")} disabled={isProcessing || !activeSubmission} className={`flex-1 py-5 border rounded-2xl font-black text-[11px] uppercase tracking-[0.25em] transition-all flex items-center justify-center gap-3 ${showRejectReason ? "bg-rose-600 border-rose-600 text-white" : "border-rose-500/50 text-rose-400 hover:bg-rose-600 hover:text-white"}`}>
