...
import IndicatorsPageIdModal from "./IndicatorsPageIdModal"; // adjust import path as needed
...
const SuperAdminApprovals = () => {
  ...
  const [selectedIndicator, setSelectedIndicator] = useState<ApprovalIndicator | null>(null);

  const { indicators, loading } = useAppSelector((state) => state.indicators);
  // allStaff is needed by IndicatorsPageIdModal — pull from your store or pass []
  const allStaff = useAppSelector((state) => state.users?.users ?? []);
    ...
    <>
                    ...
                    <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-4">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="mt-3 flex items-center gap-1 text-[9px] font-mono text-slate-400">
                          <Hash size={9} />
                          Submission ID:{" "}
                          {submissionId?.slice(-12).toUpperCase() || "N/A"}
                        </div>
                      </div>
      ...
      {selectedIndicator && (
        <div
          ...
          onClick={(e) => {
            if (e.target === e.currentTarget) setSelectedIndicator(null);
          }}
        >
          <div className="w-full max-w-2xl bg-[#fcfcf7] h-full overflow-hidden shadow-2xl animate-in slide-in-from-right duration-300">
