...
const STATUS_CONFIG: Record<string, IStatusItem> = {
  pending:   { label: "Pending",        color: "#4B5563", theme: "bg-[#F9FAFB] border-[#4B5563] text-[#4B5563]", icon: Clock },
  awaiting:  { label: "In Review",      color: "#1E3A2B", theme: "bg-[#F0FDF4] border-[#1E3A2B] text-[#1E3A2B]", icon: Send },
  rejected:  { label: "Rejected",       color: "#991B1B", theme: "bg-[#FEF2F2] border-[#991B1B] text-[#991B1B]", icon: XCircle },
  partially: { label: "Admin Verified", color: "#C69214", theme: "bg-[#FFFBEB] border-[#C69214] text-[#C69214]", icon: Gavel },
  completed: { label: "Completed",      color: "#059669", theme: "bg-[#ECFDF5] border-[#059669] text-[#059669]", icon: CheckCircle2 },
  overdue:   { label: "Overdue",        color: "#7f1d1d", theme: "bg-red-50 border-red-800 text-red-900",         icon: AlertTriangle },
};
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="border-b-2 border-[#C69214] pb-4 flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-black text-[#1E3A2B] font-serif uppercase tracking-tight">
                Performance Dashboard
              </h1>
              <p className="text-[#C69214] font-bold text-[9px] uppercase tracking-[0.2em]">
                PMMU Oversight — Kenya Judiciary
              </p>
            </div>
          </div>
          <div className="hidden sm:block text-right">
            <p className="text-[10px] font-bold text-gray-400 uppercase">System Time</p>
            <p className="font-mono text-xs font-bold text-[#1E3A2B]">
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-4">
            <div className="bg-white rounded-2xl border border-[#E5D5B0] p-6 shadow-sm text-center">
              <h3 className="text-[9px] font-black text-[#C69214] uppercase tracking-widest mb-6">
            ...
            <div className="p-5 bg-[#1E3A2B] rounded-2xl border-l-4 border-[#C69214] shadow-lg">
              ...
              <p className="text-[10px] text-white/80 leading-relaxed font-medium">
                Evidence uploads must be in PDF or Image format. Once "In Review", records are
                locked from further editing unless a correction is requested.
              </p>
            </div>
          </div>
        </div>
...
const IndicatorStatusPieChart: React.FC<{ data: IStat[] }> = ({ data }) => {
  ...
  if (!total) {
    return (
      <div className="text-[10px] text-gray-300 py-10 italic font-bold uppercase tracking-widest">
        Registry Empty
      </div>
    );
  }
