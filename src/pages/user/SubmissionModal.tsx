...
const SubmissionModal = ({ task, onClose }: SubmissionModalProps) => {
  ...
  // Initialize to the task's active quarter
  ...
  // Logic to check submission status for the selected period
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (files.length === 0) return toast.error("At least one evidence file is required.");
    ...
    // Sending the target value as achieved to mark completion for this period
    ...
    if (submitIndicatorProgress.fulfilled.match(result)) {
      ...
      toast.success(
        isRejected
          ...
          : "Evidence received. The submission timer for this period has been stopped."
      );
    ...
    <div className="fixed inset-0 z-[1000] flex justify-end overflow-hidden">
      ...
      <section className="relative w-full max-w-lg md:max-w-xl bg-[#fcfdfb] h-full shadow-2xl flex flex-col border-l border-slate-200 animate-in slide-in-from-right duration-500 ease-out">
        ...
        <div className="px-6 py-5 md:px-8 md:py-6 border-b border-slate-100 bg-white flex justify-between items-center shrink-0">
          <div className="overflow-hidden text-[#1a3a32]">
            ...
            <p className="text-[#c2a336] text-[9px] font-bold uppercase tracking-[0.15em] mt-0.5 truncate">
              {/* 🔹 FIXED: Changed indicatorTitle to activityDescription */}
              {task.activityDescription || "Performance Indicator"}
            </p>
          </div>
          <button onClick={onClose} className="p-2 hover:bg-slate-50 rounded-full transition-all group shrink-0">
            <X size={18} className="text-slate-400 group-hover:rotate-90 group-hover:text-rose-500 transition-all duration-300" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 md:p-8 no-scrollbar bg-[#f8f9fa]">
          {success ? (
            <div className="h-full flex flex-col items-center justify-center text-center space-y-4 animate-in fade-in zoom-in duration-500">
              ...
              <div>
                <h4 className="text-lg font-black text-[#1a3a32] uppercase">Submission Successful</h4>
                <p className="text-slate-400 text-[11px] font-bold uppercase tracking-wider mt-1">Registry Audit Pipeline Updated</p>
              </div>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="space-y-6">
              {isRejected && (
                <div className="bg-rose-50 border-l-4 border-rose-500 p-4 rounded-r-xl space-y-1">
                  ...
                  <p className="text-xs text-rose-600 font-medium italic">
                    "{currentPeriodSubmission?.adminComment || "Please address audit findings."}"
                  </p>
                </div>
              )}
              ...
              {!isAnnual ? (
                <div className="space-y-2.5">
                  <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest px-1">Reporting Quarter</label>
              ...
              {/* Upload */}
              <div className="space-y-2.5">
                <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest px-1">Evidence Documentation</label>
                <div className="relative group">
                  ...
                  <div className={`border-2 border-dashed rounded-xl p-6 text-center transition-all ${files.length > 0 ? "border-[#1a3a32] bg-emerald-50/30" : "border-slate-200 hover:border-[#1a3a32]/50 bg-white"}`}>
                    <Upload className={`mx-auto mb-2 transition-colors ${files.length > 0 ? "text-[#1a3a32]" : "text-slate-300 group-hover:text-[#1a3a32]"}`} size={24} />
                    <p className="text-[9px] font-black text-[#1a3a32] uppercase tracking-widest">{files.length > 0 ? `${files.length} Files Selected` : "Upload Evidence"}</p>
                  </div>
                </div>
                {files.length > 0 && (
                  <div className="grid grid-cols-1 gap-2 mt-3">
                    {files.map((f, i) => (
                      <div key={i} className="flex items-center justify-between bg-white p-2.5 px-4 rounded-lg border border-slate-100 shadow-sm">
              ...
              {/* Notes */}
              <div className="space-y-2.5">
                <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest px-1">Submission Narrative</label>
                <textarea
                  value={notes}
        ...
        <div className="px-6 py-4 md:px-8 md:py-5 bg-white border-t border-slate-100 flex items-center justify-between shrink-0">
          ...
          >
            {uploading ? <Loader2 className="animate-spin" size={14} /> : <ShieldCheck size={14} />}
