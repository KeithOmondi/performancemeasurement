...
/** * Updated to include optional title to match Service type overlap 
 */
...
const isIndicatorOverdue = (indicator: IIndicator, currentQuarter: number): boolean => {
  const status: string = indicator.status ?? "";
...
const AdminDashboardPage = () => {
  ...
  const { allAssignments: indicators = [], isLoading: iLoad } = useSelector(
    (state: RootState) => (state.adminIndicators as { allAssignments: IIndicator[]; isLoading: boolean })
  );
  
  const { stats, loading: sLoad } = useSelector((state: RootState) => state.dashboard);
  
  // Cast via unknown to handle the overlap mismatch between local and service interfaces
  const { plans = [], loading: pLoad } = useSelector(
    (state: RootState) => (state.strategicPlan as unknown as { plans: IStrategicPlan[]; loading: boolean })
  );
  ...
  const derivedData = useMemo(() => {
    const totalActivities = (plans ?? []).reduce(
      (acc: number, p: IStrategicPlan) =>
        acc +
        (p.objectives?.reduce(
          (oAcc: number, obj: IObjective) => oAcc + (obj.activities?.length ?? 0),
          0
        ) ?? 0),
      0
    );
    ...
    const overdue = indicators.filter((i) => isIndicatorOverdue(i, currentQuarter)).length;
  ...
  const pendingQueue = indicators
    .filter((ind) =>
      ["Awaiting Admin Approval", "Awaiting Super Admin", "Partially Approved"].includes(ind.status)
    )
    .slice(0, 5);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          ...
          <div className="space-y-3">
            {pendingQueue.length > 0 ? (
              pendingQueue.map((item) => {
                ...
                const isResub = (item.submissions ?? []).some((s) => s.resubmissionCount > 0 && s.reviewStatus === "Pending");
                return (
                  <div key={item.id} className="bg-white p-4 rounded-xl border border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between shadow-sm group hover:border-slate-200 transition-all">
                    <div className="flex items-center space-x-4">
                      <div className="relative">
                        <Clock size={18} className={`${isEscalated ? "text-emerald-600" : "text-yellow-500"} opacity-40`} />
                        {isResub && <div className="absolute -top-1 -right-1 w-2 h-2 bg-red-600 rounded-full animate-pulse" />}
                      </div>
                      <div>
                        <p className="text-sm font-bold text-[#1d3331] italic leading-snug line-clamp-1">"{item.activity?.description}"</p>
                        <p className="text-[10px] text-slate-400 mt-1 font-bold uppercase tracking-tighter">
                          {item.assigneeName} • <span className={isEscalated ? "text-emerald-700" : "text-yellow-600"}>{item.status}</span>
                        </p>
                      </div>
                    </div>
                    <div className="flex items-center gap-4 mt-3 sm:mt-0">
                      ...
                      <Link to={`/admin/indicators/${item.id}`} className="bg-[#1d3331] text-white p-2 rounded-lg opacity-0 group-hover:opacity-100 transition-all shadow-lg">
        ...
        <div className="lg:col-span-4">
          ...
          <div className="space-y-4">
            {(stats?.perspectiveStats ?? []).map((p: IPerspectiveStat, idx: number) => (
...
const StatCard = ({ label, value, border, textColor }: { label: string; value: number; border: string; textColor?: string }) => (
