...
/** Returns true if an indicator is overdue (deadline passed and not completed). */
const isIndicatorOverdue = (indicator: IAdminIndicator): boolean => {
  const { status, deadline } = indicator;
...
const AdminDashboardPage = () => {
  ...
  const stats = useMemo(() => {
    ...
    const pendingReview = indicators.filter(
      (i) => i.status === "Awaiting Admin Approval" || i.status === "Correction Needed"
    ).length;
    const approved = indicators.filter((i) => i.status === "Awaiting Super Admin").length;
    const returnedForCorrection = indicators.filter((i) => i.status === "Correction Needed").length;
  ...
  return (
    <div className="min-h-screen bg-[#fcfcf7] p-4 md:p-8 text-[#1a2c2c] font-sans">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
        <StatCard label="Total Indicators" value={stats.total} border="border-[#1d3331]" />
        <StatCard label="Assigned" value={stats.assigned} border="border-slate-300" />
        <StatCard label="Unassigned" value={stats.unassigned} border="border-slate-200" textColor="text-slate-400" />
        <StatCard label="Pending Action" value={stats.pendingReview} border="border-yellow-500" textColor="text-yellow-600" />
        <StatCard label="Certified Completed" value={stats.approved} border="border-emerald-700" textColor="text-emerald-700" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-4 mb-10">
        <div className="bg-white p-5 rounded-xl border-t-4 border-red-800 shadow-sm flex flex-col justify-between h-32">
          <div className="text-4xl font-serif font-bold text-red-800">{stats.overdue}</div>
        ...
        <div className="lg:col-span-2 bg-white p-6 rounded-xl border-l-[6px] border-[#1d3331] shadow-sm flex items-center h-32 group hover:shadow-md transition-all">
          <CheckCircle2 size={32} className="text-[#1d3331] mr-4 opacity-20 group-hover:opacity-40" />
          <div>
            <span className="text-4xl font-serif font-bold text-[#1d3331]">{stats.approved}</span>
        ...
        <div className="lg:col-span-2 bg-white p-6 rounded-xl border-l-[6px] border-orange-600 shadow-sm flex items-center h-32 group hover:shadow-md transition-all">
          <AlertCircle size={32} className="text-orange-600 mr-4 opacity-20 group-hover:opacity-40" />
          <div>
            <span className="text-4xl font-serif font-bold text-orange-600">{stats.returnedForCorrection}</span>
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-tight">
          ...
          <div className="space-y-3">
            {pendingQueue.length > 0 ? (
              pendingQueue.map((item) => {
                const submissions = flattenSubmissions(item.submissions);
                const isResub = submissions.some(
                  ...
                  >
                    <div className="flex items-center space-x-4">
                      <div className="relative">
                        <Clock size={18} className="text-yellow-500 opacity-40" />
                        {isResub && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          ...
          <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-5">
            ...
            <div className="grid grid-cols-7 mb-2">
              {["Su", "Mo", "Tu", "We", "Th", "Fr", "Sa"].map((d) => (
                <div
                  key={d}
                  className="text-center text-[9px] font-black text-slate-300 uppercase tracking-widest py-1"
                >
                  {d}
                </div>
              ))}
          ...
          {selectedDay !== null && (
            <div className="mt-4 bg-white rounded-xl border border-slate-100 shadow-sm p-5">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">
                {MONTH_NAMES[calMonth]} {selectedDay} — {selectedEvents.length} event
        ...
        <div className="lg:col-span-7">
          <div className="flex justify-between items-end mb-6">
            <div>
              <h3 className="text-xl font-serif font-bold text-[#1d3331]">Upcoming Deadlines</h3>
              <p className="text-[11px] text-slate-400 font-bold uppercase tracking-wider">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      >
                        {isOverdue ? (
                          ...
                          <>
                            <span className="text-[13px] font-black leading-none text-[#1d3331]">
